<template>
    <div id="transportJcModelChart">
        <div class="content">
            <search v-model="searchValue" :options="searchOption" @search="queryData(1)"></search>
            <div class='wrap'>
                <vertical-bar-chart :title="'运输车辆检测车型图表统计'" :barData="barData"></vertical-bar-chart>
            </div>
        </div>
...
export default {
    ...
    data () {
        return {
            ...
            breadArr: [
                { title: '查询统计', path: '' }, { title: '运输车辆检测车型图表统计', path: '' }
            ]
        }
    },
    components: {
